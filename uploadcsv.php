...
{
	if(uploaddatarights($_SESSION['clientemail'],$_GET['reportid'])!='1')
	{
...
<div class='container'>
	...
	<div style='text-align:right;'><a href='Dash Report Header.zip'>Download CSV Header</a></div>
	  <form method='post' action='' id='uploadcsvform' enctype="multipart/form-data"  autocomplete='off'>
		...
		<section>
				...
				<div class='col-md-3'>
					    <div class='form-group'>
						    <select name='typeofdata' required id='typeofdata' class='form-control'>
							    <option>Relevant Data</option>
					...
					<div class='col-md-3'>
				        ...
				        <div class='form-group'>
						    <input type='text' name='pdfdisplayname' id='pdfdisplayname' class='form-control' placeholder='PDF Display Name'/>
					    </div>
					</div>
					<div class='col-md-3'>
					    
		                <input type='submit' value='Save' class='button button1' style='margin:0px;' id='sbmit'/>
      ...
      <div class='msg-container-content'></div>
	  </div>
</div>
<div class='bottom'>
   ...
   <!-- jQuery-->
	...
	$(document).ready(function () {
		...
		$('#typeofdata').on('change',function(){
			if($(this).val()=='PDF')
			{
				$('#pdfdisplayname').show();
				$('#pdfdisplayname').prop('required',true);
				
			}
			else
			{
				$('#pdfdisplayname').prop('required',false);
				$('#pdfdisplayname').hide();
				$('#pdfdisplayname-error').hide();
			}
		});
		...
		$("#uploadcsvform").validate();
         ...
         $("#sbmit").click(function (e) {
					  ...
					  e.preventDefault();
					  if($('#uploadcsvform').valid()==true)
					  {
						 
					       //$('.msgpop-container').show();
						  //$('#msg-container-close-btn').prop('disabled',true);
						  var form = $('#uploadcsvform')[0];
                    var formdata = new FormData(form);
                    formdata.append("action", "uploadrelevantdata");
					formdata.append("reportid", <?php echo $_GET['reportid']?>);
					formdata.append("typeofdata", $('#typeofdata').val());
					
					if($('#typeofdata').val()=='PDF')
					{
						formdata.append("pdfdisplayname", $('#pdfdisplayname').val());
					}
                     ...
                     $.ajax({
                          ...
                          success: function (data) 
						  {
									 ...
									 if(data[0].error=='')
									 {
									  ...
									  $('.msg-container-content').html('');
									   var result =  data[0].inserterror;
									   var content ='';
									   for (i = 0; i < result.length; i++) 
								       {
										  content = content+"<tr><td>"+result[i].rowno+"</td><td>"+result[i].msg+"</td></tr>";  
									   }
									    $('.msg-container-content').html("<table class='table'><thead><th>Rowno</th><th>Message</th></thead><tbody>"+content+"</tbody></table>");
									  
									 }
									 else
									 {
							           $('.msg-container-content').html('<font color="red">ERROR OCCURED..'+data[0].error+'</font>'); 
									 }
