...
else
{
		 ...
		 if($typeofuser=='admin')
		 {
	      $sql =    "SELECT r.rid,r.type,r.title,r.reporttag,r.referenceno,r.pono,r.creationtime,r.uploadedby FROM reports r";
		 }
		 elseif($typeofuser=='user')
		 {
		   $sql =    "SELECT r.rid,r.type,r.title,r.reporttag,r.referenceno,r.pono,r.creationtime,r.uploadedby FROM reports r where r.uploadedby ='".$_SESSION["clientemail"]."'";
		 }
		 else
		 {
			 echo "<script>alert('You are not authorised to use this Module.')</script>";
			...
			for($i=0;$i<$num;$i++) 
	           {
				 ...
				 elseif($row['type']=='Corporate Site')
				 {
					 $newdata['url'] ="corporatesite.php?reportid=".$row['rid'];
				 }
				 elseif($row['type']=='Divisionsite')
				 {
					 $newdata['url'] ="divisionsite.php?reportid=".$row['rid'];
				 }
				 else
				 {
...
<div class='header'><button  class='actionbtn'  style='float:right;'><a href='newreport.php' class=''>New Report</a></button></div>
				  
<div id='middle-container-outer-wrapper-block'>
<table width="100%" id='mytble'>
<thead><th>Sr.No.</th><th>Report Title</th><th>Division/Tag</th><th>Report Type</th><th>Reference No.</th><th>PO No.</th><th>Creation Time</th><th>Uploaded by</th><th>Action</th></thead>
<tbody>
<?php
               /*for($i=0;$i<count($data);$i++) 
	           {
				 echo '<div class="dir-cont"><a target="_blank" href='.$data[$i]["url"].' class="file"><div class="dir-img"><img src="images/xlsx.jpg"></div><div class="dir-name">'.$data[$i]["title"].'</div></a></div>';
			   }*/
			   
			   for($i=0;$i<count($data);$i++) 
	           {
				 $srno =   $i+1;
				 echo '<tr><td>['.$srno.']</td><td><a target="_blank" href='.$data[$i]["url"].'>'.$data[$i]["title"].'</a></td><td>'.$data[$i]["reporttag"].'</td><td>'.$data[$i]["type"].'</td><td>'.$data[$i]["referenceno"].'</td><td>'.$data[$i]["po"].'</td><td>'.$data[$i]["creationtime"].'</td><td>'.$data[$i]["uploadedby"].'</td><td><a title="share" data-toggle="modal" class="rstatus action" data-target="#viewreportstatus" data-reportid="'.$data[$i]["rid"].'" data-reportname="'.$data[$i]["title"].'"><span><i class="fa fa-share" aria-hidden="true"></i></span></a><a class="action" title="Upload data" href="uploadcsv.php?reportid='.$data[$i]["rid"].'"><i class="fa fa-upload" aria-hidden="true"></i></a><a class="deletereport action" title="Delete" data-reportid="'.$data[$i]['rid'].'"><span><i class="fa fa-trash" aria-hidden="true"></i></span></a></td></tr>';
			   }
			   ?>
</tbody>			   
</table>	  
...
<div id="viewreportstatus" class="modal fade" role="dialog">
      ...
      <div class="modal-body">
         <form id='assignform'/>
	                       <div class='row'>
											...
											<input type='hidden' name='sharedreportname' id='sharedreportname' value=''>
						                    <div class='col-md-12'>
											                       <div class='row'>
																   <div class='col-md-3'> 
																	 ...
																	 <div class='form-group'>
																	   <label></label>
																	    <input type='submit' value='ADD' class='form-control' id='sbmit'>
																	   </div>
																	 </div>
									        							</div>			
															           <div class='row'>
																	      <div class='col-md-12'> 
											                                        ...
											                                        <div class='form-group'>
																                        <input type='checkbox' name='dontnotify' id='dontnotify' value='1'><label for='dontnotify'> Don't send email notifications while sharing reports with other</label>
																	                 </div>
										                                   </div>
																	   </div>
   ...
   <!-- jQuery-->
	...
	$(document).ready(function () {
		$('#myTopnav a').each(function(){
		 ...
		 $('body').on('click','.rstatus',function(){
			//alert($(this).data("reportid"));
			$('#reportid').val($(this).data("reportid"));
			$('#sharedreportname').val($(this).data("reportname"));
			sharedlist($(this).data("reportid"));
		});		
	    
		
		
		function sharedlist(reportid)
		{
			$.ajax({
                                type: 'POST',
                                url: 'request.php',
								data:'action=sharedlist&reportid='+reportid,
								dataType: "json",
                                success: function(data)
			                     {
								  if(data!=0)
							       ...
							       for (i = 0; i < data.length; i++) 
								      {
										    //alert(data[i].id);
                                            content = content+"<tr><td><td>"+data[i].email+"</td><td>"+data[i].right+"</td><td><a class='delete' title='Delete' data-reportid='"+reportid+"' data-allocationid="+data[i].allocation_id+"><span>Delete</span></a></td>";
											
											
											content=content+"</tr>";
                                            
									  }
									  $("#sharedlistbody").empty();
									  $("#sharedlistbody").append(content);
								   }
                                    else
									{
										$("#sharedlistbody").empty();
		...
		$("#assignform").submit(function(e)
		 {
			e.preventDefault();
					if($("#assignform").valid() == true)
		             {
						 $("#sbmit").prop("disabled", true);
						 var fd = $("#assignform").serialize();
			              $.ajax({
                                type: 'POST',
                                url: 'request.php',
								data: fd+"&action=allocateUser",
								dataType:"json",
                                success: function(data)
			                     {
									 if(data[0].accessright==0)
									 {
										 alert('Only client with owner of the report or admin have right to share this report with other.'); 
										 return false;
									 }
									 if(data[0].rid!='')
									 {
									  //
									  sharedlist(data[0].rid);
									 }
									 else
									 {
		 ...
		 $('body').on('click','.delete',function(){
				 var userid = $(this).data("allocationid");
				 var req = confirm("Are you sure you want to delete it!! ");
		           if(req)
			       {
			          $.ajax({
                                type: 'POST',
                                url: 'request.php',
								data:'userid='+userid+"&action=deallocateuser&reportid="+$(this).data("reportid"),
								dataType: "json",
                                success: function(data)
			                     {
									 ...
									 if(data[0].accessright==0)
									 {
										 alert('Only owner of the report have right to delete this user.'); 
										 return false;
									 }
									 if(data[0].rid!='')
									 {
				 ...
				 var req = confirm("Are you sure you want to delete it!! ");
		           if(req)
			       {
			          $.ajax({
                                type: 'POST',
                                url: 'request.php',
								data:'reportid='+reportid+"&action=deletereport",
								dataType: "json",
                                success: function(data)
			                     {
									 ...
									 {
									  alert('Deleted');
									  window.location.href="allfiles.php";
									 }
									 else
									 {
							           alert('Something Went Wrong');
