	...
	$("#level3").on('change',function(){
		...
		$.ajax({
					  ...
					  $.ajax({
                           type: "POST",
                           url: "retrievetext.php",
                           data: fd+"&action=getinsight&reportid="+reportid+"&chart=43",
                           timeout: 600000,
						   dataType: "json",
						   success: function (data) 
						   {
							  $('#insightdescription').html("Top citing companies segrated as A and B"); 
							  $('#processing-div').remove();
							 	 if(data!=0)
								  {
									  ...
									  for (i = 0; i < data.length; i++) 
								      {
										  x.push(data[i].citingowner);
										  z.push(data[i].count);
										  y.push(data[i].citationtype);
									  }
									  
									        globalx = x;
											globaly  = y;
											globalz = z;
											//globalmetadata ={'drilldown':'disabled','x':'innovationcountry','y':'assignee','xcol':'Country of innovation','ycol':'Assignee Name','zcol':'Count'};
											
											changecharttype('3Dbar','Top thirty overall citing companies','chart-3');
							       }
								   else
								   {
	                   ...
	                   $.ajax({
                           type: "POST",
                           url: "retrievetext.php",
                           data: fd+"&action=getinsight&reportid="+reportid+"&chart=42&citationtype=A",
                           timeout: 600000,
						   dataType: "json",
						   success: function (data) 
						   {
							    $('#insightdescription').html("This chart illustrates top twenty citing companies of Corprate patents");
							    $('#processing-div').remove();
							    if(data!=0)
								  {
									  exportdata = data;
									  ...
									  for (i = 0; i < data.length; i++) 
								      {
										 ...
										 changecharttype('hbar','Top twenty citing companies of citation type -A','chart-1');
					  ...
					  $.ajax({
                           type: "POST",
                           url: "retrievetext.php",
                           data: fd+"&action=getinsight&reportid="+reportid+"&chart=42&citationtype=B",
                           timeout: 600000,
						   dataType: "json",
						   success: function (data) 
						   {
							    ...
							    if(data!=0)
								  {
									  ...
									  for (i = 0; i < data.length; i++) 
								      {
										 ...
										 changecharttype('hbar','Top twenty citing companies of citation type -B','chart-2');
