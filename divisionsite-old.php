...
//---------------get list of all bench marking projects--------------------
        ...
        mysqli_select_db($conn,$dbname);
		 if($typeofuser =='admin')
		 {
		 $sql =  "SELECT r.rid,r.type,r.title,r.reporttag,r.referenceno,r.pono,r.creationtime,r.uploadedby FROM reports r where r.type = 'Divisionsite'";	 
		 }
		 else
		 {	 
		 $sql =  "SELECT r.rid,r.type,r.title,r.reporttag,r.referenceno,r.pono,r.creationtime,r.uploadedby FROM reports r,reportallocation allo where r.rid=allo.rid && allo.email='".$_SESSION["clientemail"]."' && r.type = 'Divisionsite'";
		 }
		 $result = mysqli_query($conn, $sql);
		 $num = mysqli_num_rows($result);
...
#insight-result-count-clickedpoint
{
	display: inline;
    margin: 5px;
	float:right;
...
<?php
 ...
 <!-- <div class='header'><button  class='actionbtn'  style='float:right;'><a target='_blank' id='exportbtn' href='exportresultset.php?reportid=<?php echo$_GET["reportid"]?>' title='Export Resultset In Excel Sheet'>Export</a></button><button title='Show result set' class='actionbtn' id='bcktolst' style='float:right;'>Result set</button><button title='show/hide filters' class='actionbtn filter-area-not-expanded' id='showfilters' style='float:right;'>Filters</button><button title='show/hide comments' class='actionbtn comment-area-not-expanded' id='showcomments' style='float:right;'>Comments</button><button title='show insights' class='actionbtn' id='showinsights' style='float:right;'>Insights</button></div>-->
  ...
  <div class='site-header'>Division Site <a class='gotoexternal' title='Export Scores' target='_blank'>Export Data <i class="fa fa-external-link" aria-hidden="true"></i></a></div>
  ...
  <div class='insight-card-wd-100'><div class='insight-card-inner' id='chart-1'></div></div>
  <!--<div class='insight-card-wd-100'><div class='insight-card-inner' id='chart-2'></div></div>-->
  <div class='insight-card-wd-100'><div class='insight-card-inner' id='chart-3'></div></div>
  <div class='insight-card-wd-100'><div class='insight-card-inner' id='chart-4'></div></div>
					...
					function corporateinsights()
					{
                       ...
                       $('#chart-6').html("<div id='processing-div-6'>We are processing your request.... <img src='images/process.gif'/></div>");					   
						$.ajax({
                           type: "POST",
                           url: "retrievetext.php",
                           data: fd+"&action=getinsight&reportid="+reportid+"&chart=50",
                           timeout: 600000,
						   dataType: "json",
						   success: function (data) 
						   {
							    $('#insightdescription').html("This chart illustrates the dominant companies according to patent portfolio size.");
							    ...
							    if(data!=0)
								  {
									  ...
									  for (i = 0; i < data.length; i++) 
								      {
										  ...
										  z.push(data[i].count);
											...
											globaly  = y;
											globalz = z;
											var globalmetadata ={'drilldown':'enable','x':'pdate','y':'assignee'};
											
											
										 changecharttype('3Dbar','Patent Portfolio Size','chart-1',globalmetadata);
					  ...
					  $.ajax({
                           type: "POST",
                           url: "retrievetext.php",
                           data: fd+"&action=getinsight&reportid="+reportid+"&chart=51",
                           timeout: 600000,
						   dataType: "json",
						   success: function (data) 
						   {
							    $('#insightdescription').html("This chart illustrates the dominant companies according to patent asset score.");
							    ...
							    if(data!=0)
								  {
									  ...
									  for (i = 0; i < data.length; i++) 
								      {
										 ...
										 changecharttype('3Dbar','Patent Portfolio Asset value','chart-3',globalmetadata);
	                   ...
	                   $.ajax({
                           type: "POST",
                           url: "retrievetext.php",
                           data: fd+"&action=getinsight&reportid="+reportid+"&chart=4",
                           timeout: 600000,
						   dataType: "json",
						   success: function (data) 
						   {
                     ...
                     $.ajax({
                           type: "POST",
                           url: "retrievetext.php",
                           data: fd+"&action=getinsight&reportid="+reportid+"&chart=52",
                           timeout: 600000,
						   dataType: "json",
						   success: function (data) 
						   {
							    $('#insightdescription').html("Market Coverage");
							    ...
							    if(data!=0)
								  {
									  ...
									  for (i = 0; i < data.length; i++) 
								      {
										 ...
										 changecharttype('3Dbar','Market Coverage','chart-4',globalmetadata);
					  ...
					  $.ajax({
                           type: "POST",
                           url: "retrievetext.php",
                           data: fd+"&action=getinsight&reportid="+reportid+"&chart=53",
                           timeout: 600000,
						   dataType: "json",
						   success: function (data) 
						   {
							   
							   $("#insightdescription").html("ETR");
									 ...
									 for (i = 0; i < data.length; i++) 
								      {
										  ...
										  z.push(data[i].score);
											...
											globaly  = y;
											globalz = z;
											var globalmetadata ={'drilldown':'enable','x':'pdate','y':'assignee','xcol':'Priority Date','ycol':'Assignee','zcol':'Average of ETR'};
											
											
										changecharttype('3Dbar','External Technology Relevance','chart-5',globalmetadata);
					  ...
					  $.ajax({
                           type: "POST",
                           url: "retrievetext.php",
                           data: fd+"&action=getinsight&reportid="+reportid+"&chart=54",
                           timeout: 600000,
						   dataType: "json",
						   success: function (data) 
						   {
							   
							   $("#insightdescription").html("ECI");
									 ...
									 for (i = 0; i < data.length; i++) 
								      {
										  ...
										  z.push(data[i].score);
											...
											globaly  = y;
											globalz = z;
											var globalmetadata ={'drilldown':'enable','x':'pdate','y':'assignee','xcol':'Priority Date','ycol':'Parent Assignee','zcol':'Avg. of ECI'};
											
											
										changecharttype('3Dbar','ECI','chart-6',globalmetadata);
