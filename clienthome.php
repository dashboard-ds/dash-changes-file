...
else
{
         mysqli_select_db($conn,$dbname);
	      ...
	      $sql =    "SELECT distinct r.type,r.reporttag,count(*) as noofreports FROM reports r,reportallocation allo where r.rid=allo.rid && allo.email='".$_SESSION["clientemail"]."' group by r.type,r.reporttag";
		  //echo $sql;
			...
			for($i=0;$i<$num;$i++) 
	           {
				 ...
				 $newdata['count'] = $row['noofreports'];
				 $newdata['url'] ="clientfiles.php?divisiontype=".rawurlencode($row['reporttag'])."&type=".rawurlencode($row['type']);
				 if($row['type']=='Patent Mining')
				 {
					 $newdata['class'] ="PatentMining";
				 }
				 elseif($row['type']=='Benchmarking')
				 {
					 $newdata['class'] ="BenchMarking";
				 }
				 elseif($row['type']=='FTO')
				 {
					 $newdata['class'] ="FTO";
				 }
				 elseif($row['type']=='Landscape')
				 {
					 $newdata['class'] ="Landscape";
				 }
				 elseif($row['type']=='Divisionsite')
				 {
					 $newdata['class'] ="Divisionsite";
				 }
				 elseif($row['type']=='Patent Citation')
				 {
					 $newdata['class'] ="Patentcitation";
				 }
				 else
				 {
					 $newdata['class'] ="CorporateSite";
				 }
...
<div class="site-header"><a class="gotoexternal" href='ipsearchindicator.php' title="Statistics page for overall search performed" target="_blank">Project Statistics <i class="fa fa-external-link" aria-hidden="true"></i></a></div>
...
<?php
	           ...
	           {
				 $srno =   $i+1;
				 if(isset($data[$i]["type"]) && trim($data[$i]["type"])!='' && isset($data[$i]["reporttag"]) && trim($data[$i]["reporttag"])!='')
				 {
					 echo "<div class='typeblock ".$data[$i]["class"]."' data-url='".$data[$i]["url"]."'><div><span class='typeblock-header'>Report Type:</span><span class='typeblock-value'>".$data[$i]["type"]."</span></div> <div><span class='typeblock-header'>Division/Tag:</span><span class='typeblock-value'>#".$data[$i]["reporttag"]."</span></div> <div><span class='typeblock-header'>Count:</span><span class='typeblock-value'>".$data[$i]["count"]."</span></div></div>";
				 }
				 elseif(isset($data[$i]["reporttag"]) && trim($data[$i]["reporttag"])!='')
				 {
					echo "<div class='typeblock ".$data[$i]["class"]."' data-url='".$data[$i]["url"]."'> <div><span class='typeblock-header'>Division/Tag:</span><span class='typeblock-value'>#".$data[$i]["reporttag"]."</span></div> <div><span class='typeblock-header'>Count:</span><span class='typeblock-value'>".$data[$i]["count"]."</span></div></div>";
				 }
				 elseif(isset($data[$i]["type"]) &&  trim($data[$i]["type"])!='')
				 {
					echo "<div class='typeblock ".$data[$i]["class"]."' data-url='".$data[$i]["url"]."'><div><span class='typeblock-header'>Report Type:</span><span class='typeblock-value'>".$data[$i]["type"]."</span></div> <div><span class='typeblock-header'>Count:</span><span class='typeblock-value'>".$data[$i]["count"]."</span></div> </div>"; 
				 }
...
<div class='bottom'>
   ...
   <!-- jQuery-->
	...
	$(document).ready(function () {
		...
		$('body').on('click','.typeblock',function(){
