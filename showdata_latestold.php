...
<div class='mc-pubview-navigation'>
  ...
  <div id='middle-container-consolidate-view' class='middle-container-consolidate-view visibility-true'>
	 ...
	 <?php
			  ...
			  {
				  $noofrecords = count($data['records']);
				  $records = $data['records'];
				   for($i=0;$i<count($records);$i++)
				   {
					   $relevancy = '';
					                    if($records[$i]['relevancy']!='' && $records[$i]['relevancy']!=null)
										  {
											  $relevancy = ' ('.$records[$i]['relevancy'].')';
										  }
										  $memberlegalstatus='';
										  if($records[$i]['relevantpatent_legalstatus']!='' && $records[$i]['relevantpatent_legalstatus']!=null)
										  {
											   $memberlegalstatus = '<br/>'.$records[$i]['relevantpatent_legalstatus'];
										  }
					                     echo "<tr><td class='srno'>[".$records[$i]['srno']."]</td><td class='headcol'><a class='viewpub' title='View Publication' data-listrowid='".$records[$i]['srno']."' data-pubno='".$records[$i]['pubno']."' data-reportid='".$records[$i]['rid']."'>".$records[$i]['pubno']."".$relevancy."</a>".$memberlegalstatus."</td><td>".$records[$i]['pubtitle']."</td><td>".$records[$i]['appdate']."</td><td>".$records[$i]['pdate']."<td>".$records[$i]['pubdate']."</td><td>".$records[$i]['parentassignee']."</td><td>".$records[$i]['updateddate']."</td><td id='list-tagging-".$records[$i]['srno']."'>".$records[$i]['tagging']."</td></tr>";
				   }
			  }
			  else
			  {
