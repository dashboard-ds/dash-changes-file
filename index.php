<?php 							//start php code from here
session_start();
include('function/function.php'); //include 'function.php' page to get data from that page
$warningmsg ="";
$num =0;
                
				if(!empty($_POST['uid']) && !empty($_POST['pwd']) && isset($_POST['g-recaptcha-response'])) // if condition to chek user id and password empty or not
				{
                ...
                //var_dump($gresponse);
				//exit;
				if($gresponse['success'] == true) // check captcha is true or false.
                {
				  					  				  
		          
					   
					 include 'dbconnection.php'; // include data base here for access data base in this code.

                    if(!$conn)  // check condition for connection
		            {
                    die("Connection failed: " . mysqli_connect_error());	// code for check database connection connected or not.
                    }
                    mysqli_select_db($conn,$dbname); //function is used to change the default database for the connection.

					//query for render data from  data base.
					...
					if($stmt = mysqli_prepare($conn, $sql))  //function is used to prepare an SQL statement for execution
			        {
                     mysqli_stmt_bind_param($stmt, "ss", $emailid,$cpassword);
				     $emailid = trim($_POST["uid"]);			//  put user name  to $email from uid input box
					 $cpassword = md5(trim($_POST['pwd']));		//	put user name  to $email from uid input box
			        }
					
					/* accepts a prepared statement object (created using the prepare() function) as a parameter, and executes it */

					if (mysqli_stmt_execute($stmt))
			        {			/* mysqli_stmt_bind_result() used to bind the columns of a result set to variables */
				       mysqli_stmt_bind_result($stmt,$clientid,$name,$email,$firm,$usertype,$theme);
				       $row = array();
				       $num =0;
					   /*mysqli_stmt_fetch() function returns TRUE if the data is fetched, FALSE in case of an error and, NULL if there are no more rows in the result.*/
					   ...
					   if ( $num> 0) 
                        {
								// associative array that contains all session variables
			    ...
			    else			//msg for authentication issue for captcha
				{
...
?>
<!--*************************** HTML START **********************-->
<!doctype html>  		<!-- define doctype as html -->
<html lang="en">		<!-- language define as english -->
...
<!-- ******************** head tag start ******************-->
  <head>					
    ...
    <title>Secure Login -iCuerious Dashboard</title>
	 <script src="https://www.google.com/recaptcha/api.js" async defer></script> <!-- recaptcha api added here for authentication -->
	...
	<style>
	html,body{						/* dfine html and body properties height, width , margin and padding */
	height: 100%;
    width: 100%;
	margin:0px;
	padding:0px;	
	...
	.wrapper 						/* main wrapper div for all page properties define here- align-content, height, width */
	{
	display: flex;
    flex-direction: row;
    flex-wrap: nowrap;
	...
	.login-left						/* define left side div  background image and content styles */
	{
		flex-grow:1;
	...
	.login-right					/* define right side div  content style */
	{
		flex-grow:1;
		padding:40px;
		flex-basis:0px;
	...
	.login-right-inner				/* define right side inner div  background image, welcome note ,heading  and login form styles */
	{
	display: flex;
	...
	.test{ 									/* set left padding of right side test div , inside of login-rigth-inner div */
		padding-left:80px;
	}

	.login-form								/* set right side login-form  style background , width , padding , broder-radius and top margin */
	{
	...
	.form-signin .form-control {		/* input text style define under form signin : form-control : position,font-size ,height ,padding, border radius and bottom magin */
    position: relative;
    font-size: 16px;
    height: auto;
    padding: 20px;
...
.form-signin input[type="password"] { 		/* input style define under form signin : form-control :  border radius and bottom magin */
    margin-bottom: 10px;
   border-radius:35px;
}

.form-signin {				/* define form-signin styles padding , margin and max-width */
    max-width: 99%;
    padding: 0px;
    margin: 0 auto;
}

.mybtn						/* define form "sign in" button styles */
{
	border-radius: 40px;
}

@font-face {font-family: 'Telegraf-UltraLight';src: url('css/font/Telegraf-UltraLight.otf'); }	 /* define all page font-family style */

.test svg					/* right side dashboard heading svg img height under test div */
{
	height:50px;
}

.trademarkline,.w-line 			/* define trademark link style font-family and colour */
{
  ...
  <!--*************************** BODY TAG START **********************-->
  <body>
  <div class='wrapper'>						<!-- most outer div wrapper -->
		<!-- left side div define for imag -->
      <div class='login-left'></div>		
	  <!-- right side div define for heading and login form -->
	  <div class='login-right'>		
					<!-- right side inner div define for login form ,heading and image -->		
	          <div class='login-right-inner'>
			       <div style='text-align:right;'>
				   		<img src='images/login-1.svg' height='50px'>	<!-- Right side logo image -->
					</div> 

				   <div class='test'>
				       <div class='w-line'>Welcome to</div>
					   <div>
					   		<!-- import dashboard heading and graphics form  "http://www.w3.org/2000/svg" (W3C) -->	
					   ...
					   <div class='trademarkline'>Dynamic, Collaborative, & Secure Patent Insights</div>
								<!-- define login form div -->
					   <div class='login-form'>		
									<!-- define login form for user -->
					     <form id='form-signin' class="form-signin" method="post" action="" autocomplete='off'>
                			<input type="email" class="form-control" name="uid" placeholder="Email" required> <!-- input for user id -->
                			<input type="password" class="form-control" name="pwd" placeholder="Password" required>  <!-- input for user password -->
										<!-- here we add capatcha for authenticate user or robot -->
							...
							<button class="btn btn-lg btn-primary btn-block mybtn">Sign in</button>
									...
									<!-- warning message for authenticate user and show warning if it is not valid -->
				  			<?php			
				       			echo $warningmsg; 
				   			?>
                		</form>
    ...
    <script src="js/jquery.min.js"></script>	<!-- link to import jquery.min.js -->
    <script src="js/bootstrap.min.js"></script>		<!-- link to import bootstrap.min.js -->
	<script src='js/jquery.validate.js'></script>		<!-- link to import jquery.validate.js -->

	<script>
	$("#form-signin").on("submit", function(e) {	// define function for submit captcha 
		 e.preventDefault();
    ...
    $form.submit();
       ...
       function onSubmit(token) {						// define function for submit inputs from user
