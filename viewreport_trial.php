...
mysqli_select_db($conn,$dbname);
	  ...
	  $typeofuser = checktypeofuser($_SESSION["clientemail"]);
		   ...
		   {
		   $sql ="select r.rid,r.title,r.type,r.creationtime,r.report_relevancy,r.report_scoring,r.report_categorization,r.report_typeofassignee,r.report_familymembers_legalstatus,r.report_ipccpc,r.report_headquarter,r.pdf from reports r, reportallocation allo where r.rid = ? && r.rid=allo.rid && allo.email='".$_SESSION["clientemail"]."'";
		   }
		   else if($typeofuser =='user')
		   {
			 $sql ="select r.rid,r.title,r.type,r.creationtime,r.report_relevancy,r.report_scoring,r.report_categorization,r.report_typeofassignee,r.report_familymembers_legalstatus,r.report_ipccpc,r.report_headquarter,r.pdf from reports r, reportallocation allo where r.rid = ? && r.uploadedby='".$_SESSION["clientemail"]."'";   
		   }
		   elseif($typeofuser =='admin')
		   {
			  $sql ="select r.rid,r.title,r.type,r.creationtime,r.report_relevancy,r.report_scoring,r.report_categorization,r.report_typeofassignee,r.report_familymembers_legalstatus,r.report_ipccpc,r.report_headquarter,r.pdf from reports r where r.rid = ?";    
		   }
		   else
		   {
			   echo "access denied";
		   ...
		   mysqli_stmt_bind_result($stmt,$rid,$title,$type,$creationtime,$report_relevancy,$report_scoring,$report_categorization,$report_typeofassignee,$report_familymembers_legalstatus,$report_ipccpc,$report_headquarter,$pdf);	
           $num=0;		   
...
<g>
  ...
  <button title='show/hide comments' class='actionbtn comment-area-not-expanded' id='showcomments' style='float:right;'>Comments</button><button title='show insights' class='actionbtn' id='showinsights' style='float:right;'>Insights</button>
  ...
  <?php
	   ...
	   {
		   ?>
		     <button  class='actionbtn' id='showdocs' style='float:right;'><a id='showoverallinsights' title='Open all attached Documents' title=''>Documnets</a></button>
		   <?php
	   }
  ?>
  </div>
