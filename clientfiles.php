...
else
{
		 ...
		 $filterparameter ='';
		 if(isset($_GET['divisiontype']) && isset($_GET['type']) && trim($_GET['type'])!='')
		 {
			  $filterparameter = "r.reporttag = '".$_GET['divisiontype']."' && r.type = '".$_GET['type']."'";
		 }
		 else if(isset($_GET['divisiontype']))
		 {
			 $filterparameter = "r.reporttag = '".$_GET['divisiontype']."'";
		 }
		 else if(isset($_GET['type']) && trim($_GET['type'])!='')
		 {
			 $filterparameter = "r.type = '".$_GET['type']."'";
		 }
		 
		  if($filterparameter!='')
		  {
	      $sql =    "SELECT r.rid,r.type,r.title,r.reporttag,r.referenceno,r.pono,r.creationtime,r.uploadedby FROM reports r,reportallocation allo where r.rid=allo.rid && allo.email='".$_SESSION["clientemail"]."' && ".$filterparameter;
		  }
		  else
		  {
			$sql =    "SELECT r.rid,r.type,r.title,r.reporttag,r.referenceno,r.pono,r.creationtime,r.uploadedby FROM reports r,reportallocation allo where r.rid=allo.rid && allo.email='".$_SESSION["clientemail"]."'";  
		  }
		  //echo $sql;
...
<thead><th>Sr.No.</th><th>Report Title</th><th>Division/Tag</th><th>Report Type</th><th>Reference No.</th><th>PO No.</th><th>Creation Time</th><th>Uploaded by</th><th></th></thead>
<tbody>
<?php
	           ...
	           {
				 $srno =   $i+1;
				 echo '<tr><td>['.$srno.']</td><td><a target="_blank" href='.$data[$i]["url"].'>'.$data[$i]["title"].'</a></td><td>'.$data[$i]["reporttag"].'</td><td>'.$data[$i]["type"].'</td><td>'.$data[$i]["referenceno"].'</td><td>'.$data[$i]["po"].'</td><td>'.$data[$i]["creationtime"].'</td><td>'.$data[$i]["uploadedby"].'</td><td><a data-toggle="modal" class="rstatus" data-target="#viewreportstatus" data-reportid="'.$data[$i]["rid"].'" data-reportname="'.$data[$i]["title"].'">Share</a></td></tr>';
			   }
			   ?>
</tbody>			   
</table>
  ...
  <div class="modal-dialog modal-lg">
   ...
   <!-- jQuery-->
    ...
    <script src="js/bootstrap.min.js"></script>
	 ...
	 <!---navbar js---------->
	 <script src="js/clientnavbar.js"></script>
	 <!----------bootstrap multiple----->
								  ...
								  if(data!=0)
								   {
									 if(data[0].accessright==0)
									 {
										 ...
										 window.location ="clientfiles.php";
										 return false;
									 }
									 var content ="";
                                ...
                                success: function(data)
			                     {
									 if(data[0].accessright==0)
									 {
										 alert('Only owner of the report have right to share this report with other.'); 
										 return false;
									 }
									 if(data[0].rid!='')
									 {
