...
<div class='container'>
	  ...
	  <form method='post' action='' id='myform'>
		...
		<section>
			...
			<div class='row'>
             <div class='col-md-6'>	
             <input type='hidden' readonly name='userid' class='form-control' id='userid' required value='<?php echo $_GET['userid']?>'/>				 
			...
			<div class='form-group'>
			     <input type='checkbox' name='passwordchng' id='passwordchng'/><label for='passwordchng'>Change Password</label>
				 <input type='password' name='password' class='form-control' id='password' value="" placeholder='New Password' style='display:none;'/>
			</div>		
			</div>
	    </div>
		</section>
		...
		<div class='nxtrow' style='text-align:center'>
		    <a class='button button1' href='userlist.php'>Cancel</a>
	...
	$(document).ready(function () {
		...
		$("#myform").validate();
	     ...
	     $.ajax({
                                type: 'POST',
                                url: 'request.php',
								data:'userid=<?php echo $_GET['userid']?>&action=userinfo',
								dataType: "json",
                                ...
                                success: function(data)
			                     {
									 ...
									 {
									  alert('Done');
									  window.location.href="useredit.php?userid="+data[0].id;
        ...
        $("#passwordchng").on("change",function(){
			 $("#password").toggle();
