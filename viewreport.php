...
if (isset($_SESSION["clientemail"])) {
	...
	//---------------get list of all report shared user--------------------
	include 'dbconnection.php';
	$data = array();
	mysqli_select_db($conn, $dbname);
	if ($typeofuser == 'admin') {
		$sql =  "SELECT r.rid,r.type,r.title,r.reporttag,r.referenceno,r.pono,r.creationtime,r.uploadedby FROM reports r where r.title like '%" . $reportmetainfo['title'] . "%'";
	} else {
		$sql =  "SELECT r.rid,r.type,r.title,r.reporttag,r.referenceno,r.pono,r.creationtime,r.uploadedby FROM reports r,reportallocation allo where r.rid=allo.rid && r.title like '%" . $reportmetainfo['title'] . "%' && allo.email='" . $_SESSION["clientemail"] . "'";
	}
	//echo $sql;
	$result = mysqli_query($conn, $sql);
	$num = mysqli_num_rows($result);
	...
	//list of users to whome this report is shared with		
	include 'dbconnection.php';
	$mentiondata = array();
	mysqli_select_db($conn, $dbname);

	$sql =  "SELECT c.name,c.email,c.firm FROM client c,reportallocation r where c.email=r.email && r.rid='" . trim($_GET['reportid']) . "'";
	...
	if ($num > 0) {
		for ($i = 0; $i < $num; $i++) {
			$newdata = array();
			$row = mysqli_fetch_array($result);
			$newdata['key'] = $row['name'];
			$newdata['value'] = $row['name'];
			$newdata['email'] = $row['email'];

			array_push($mentiondata, $newdata);
		}
	} else {
	}
		...
		<div class='wrapper'>
			...
			<div class='new-header'>
				<div class='new-header-child'><a href='clienthome.php'>
						...
						?>
							<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 664.94 105.93" style="enable-background:new 0 0 664.94 105.93;" xml:space="preserve">
								<style type="text/css">
									.st0 {
										fill: #EF3C18;
									}
			...
			c0.43,1.04,0.65,2.04,0.65,3.04v3.29H119.04z" />
				...
				<nav class="navbar navbelicon">
					...
					<li>
						...
						<i class="fas fa-bell fa-2x bell-icon" aria-hidden="true" style="position: absolute; right: 20%;"></i><br/>
			...
			<div class='header'>
				<button class='actionbtn' style='float:right;'><a target='_blank' id='exportbtn' href='exportresultset.php?reportid=<?php echo $_GET["reportid"] ?>' title='Export Resultset In Excel Sheet'>Export</a></button>
				<button title='Show result set' class='actionbtn' id='bcktolst' style='float:right;'>Result set</button><button title='show/hide filters' class='actionbtn filter-area-not-expanded' id='showfilters' style='float:right;'>Filters</button>
				<div>
				<?php $sql = "SELECT * FROM comment where status ='1' order by 'cid' desc";
						$resp = mysqli_query($conn, $sql); 
					
						$current_user_email=$_SESSION["clientemail"];
						$current_userr_count=0;
					...
					// 		echo '<br />';
								...
								// $jsonD = $row['useremails'];
								// $stringToArray = $array = explode(',', $jsonD);
								// $length = count($stringToArray);
								// $countUser = 0;
								// for ($i = 0; $i < $length; $i++) {
								// $abc = json_decode($array[$i], true);
								// /* pass current user email below */
								// 	if($abc['harish@designingsolutions.co.in'] == 0){
								// 		$countUser++;
								// 	}
								// }
								// echo "Result: ".$countUser;
							
						?>
					<button title='show/hide comments' u_id="<?php echo $_SESSION["clientname"]; ?>" class='actionbtn comment-area-not-expanded' id='showcomments' style='float:right;'>Comments
					</button>
				</div>
				<button title='show insights' class='actionbtn' id='showinsights' style='float:right;'>Insights</button>
				<button class='actionbtn' style='float:right;'><a id='showoverallinsights' title='A bird eye view of quick insights' target='_blank' href='insightoverview.php?reportid=<?php echo $_GET["reportid"] ?>' title=''>Insight Synopsis</a></button>
			</div>
				...
				<div id='middle-container-left-wrapper'>
					<div id='middle-container' class='middle-container'>
						<div class='mc-view-publication-inner'>
							<div class='mc-left'>
								<div class='mc-left-pubno'>Publication No.<br /><span id='pubno'></span> <span id='relevancy'></span><span id='pubtagging'></span>
									<div id='alllinks'></div>
								</div>
	      ...
	      <div class='mc-middle-relevant-claim'>
					...
					<div id='middle-container-consolidate-view' class='middle-container-consolidate-view visibility-true'>
						<div class='mc-cv-list-view'>
							...
							<table width='100%'>
								<thead>
									<th>Sr. No.</th>
									<th class='headcol' style="">Publication Number</th>
									<th style='width:300px;'>Title</th>
									<th>App. Date</th>
									<th>Priority Date</th>
									<th>Publication Date</th>
									<th>Applicant/Assignee</th>
									<th>Archieve Date</th>
									<th>Tagging</th>
								</thead>
					...
					<div class='middle-container-insight-view' id='middle-container-insight-view'>
							...
							<div class='mc-iv-left'>
								...
								<ul>
									<li>
										...
										<ul>
											<?php if ($reportmetainfo['categorization'] == 1) { ?>
												<li>
													<div class='insight-group show-help' data-tipso="This section illustrates the patent distribution among the various technology areas considered for the analysis. These charts may help identify the focused technology areas in the subject technology domain.">MAJOR TECHNOLOGY FOCUS</div>
													<ul>
														<?php
											...
											<?php } ?>
											<li>
												<div class='insight-group show-help' data-tipso="This section indicates the pace of innovation by correlating the patent filings in various technology areas/ sub-categories with the filing dates.">PACE OF INNOVATION</div>
												<ul>
													<li>
														<div class='getinsight' id='ins-4' data-chart='3'>Overall filing trends</div>
													</li>
													<?php
													...
													if ($reportmetainfo['type'] == 'FTO') { ?>
														<li>
															<div class='getinsight' id='ins-29' data-chart='29'>Expiry trends</div>
														</li>
														<?php
														if ($reportmetainfo['categorization'] == 1) {
									...
									<li>
										...
										<ul>
											<?php if ($reportmetainfo['typeofassignee'] == 1) { ?>
												<li>
													<div class='getinsight' id='ins-9' data-chart='11'>Business type of assignees</div>
												</li>
											<?php } ?>
											<li>
												<div class='getinsight' id='ins-8' data-chart='4'>Major assignees (Overall)</div>
											</li>
											<?php
											...
											?>
											<li>
												<div class='getinsight' id='ins-comp-pr-14' data-chart='14'>Companies vs Priority year</div>
											</li>
											<?php if ($reportmetainfo['typeofassignee'] == 1) { ?>
												<li>
													<div class='getinsight' id='ins-typeassignee-pr-14' data-chart='35'>Business type of assignees vs Priority year</div>
												</li>
											<?php } ?>
								...
								<?php if (($reportmetainfo['typeofassignee'] == 1) && ($reportmetainfo['headquarter'] == 1)) { ?>
									...
									<ul>
										<li>
											...
											<ul>
												<li>
													<div class='getinsight' id='ins-40' data-chart='40'>Players transition</div>
												</li>
												<li>
													<div class='getinsight' id='ins-41' data-chart='41'>Players region transition</div>
												</li>
											</ul>
										</li>
									</ul>
								...
								<ul>
									<li>
										...
										<ul>
											<li>
												<div class='getinsight' id='ins-6' data-chart='7'>Origin of innovation</div>
											</li>
											<li>
												<div class='getinsight' id='ins-7' data-chart='8'>Countries vs. Priority years</div>
											</li>
											<li>
												<div class='getinsight' id='ins-5' data-chart='6'>Preferred market countries</div>
											</li>
								...
								<ul>
									<li>
										...
										<ul>
											<li>
												<div class='getinsight' id='ins-19' data-chart='19'>Origin of innovation</div>
											</li>
											<li>
												<div class='getinsight' id='ins-20' data-chart='20'>Preferred market countries</div>
											</li>
										</ul>
									</li>
								</ul>
								...
								<ul>
									<li>
										...
										<ul>
											<li>
												<div class='getinsight' id='ins-21' data-chart='21'>Overall legal status</div>
											</li>
											<?php if ($reportmetainfo['typeofassignee'] == 1) { ?>
												<li>
													<div class='getinsight' id='ins-22' data-chart='22'>Business type of assignee</div>
												</li>
											<?php } ?>
											<li>
												<div class='getinsight' id='ins-23' data-chart='23'>Major Assignee</div>
											</li>
									...
									<?php if ($reportmetainfo['categorization'] == 1) { ?>
										<li>
											<div class='insight-group'>Technology areas vs Legal status</div>
											<ul>
												<?php
												for ($i = 0; $i < count($child); $i++) {
								...
								<?php if ($reportmetainfo['legalstatus'] == 1) { ?>
									<ul>
										<li>
											...
											<ul>
												<li>
													<div class='getinsight' id='ins-26' data-chart='26'>Overall legal status</div>
												</li>
												<li>
													<div class='getinsight' id='ins-27' data-chart='27'>Major Assignee</div>
												</li>
												<li>
													<div class='getinsight' id='ins-28' data-chart='28'>Authorities protected</div>
												</li>
											</ul>
										</li>
								...
								<?php if ($reportmetainfo['ipccpc'] == 1) { ?>
									<ul>
										<li>
											...
											<ul>
												<li>
													<div class='getinsight' id='ins-31' data-chart='31'>Major IPC classes</div>
												</li>
												<li>
													<div class='getinsight' id='ins-32' data-chart='32'>Major IPC classes VS. Companies</div>
												</li>
									...
									<ul>
										<li>
											...
											<ul>
												<li>
													<div class='getinsight' id='ins-33' data-chart='33'>Major CPC classes</div>
												</li>
												<li>
													<div class='getinsight' id='ins-34' data-chart='34'>Major CPC classes VS. Companies</div>
												</li>
										...
										<li>
											...
											<ul>
												<li>
													...
													<ul>
														<li>
															<div class='getinsight' id='ins-18' data-chart='18'>Overall</div>
														</li>
														<?php
														if ($reportmetainfo['typeofassignee'] == 1) {
															include 'dbconnection.php';
						...
						<div class='mc-iv-right'>
							...
							<div id='insight-suboption'>
								...
								<div id='switch-charts'>
									<select id="switchchart"></select>
								</div>
								<?php if (count($data) > 0) {
								?>
									<div id='comparison-report'>
										<select id="comparison-reportid">
											<option value=''>Compare report</option>
											<?php
											for ($i = 0; $i < count($data); $i++) {
												$srno = $i + 1;
												if ($_GET['reportid'] == $data[$i]["rid"]) {
												} else {
													echo '<option value="' . $data[$i]["rid"] . '">' . $data[$i]["title"] . '</option>';
												}
											}
											?>
										</select>
								...
								<div id='pdate-filter'>
									<select id="priority-year-opt">
										<option value="5">Last 5 years</option>
										<option value="7">Last 7 years</option>
										<option value="10" selected>Last 10 years</option>
										<option value="20">Last 20 years</option>
										<option value='precovid'>Pre covid years</option>
										<option value='postcovid'>Post covid years</option>
										<option value="all">All years</option>
									</select>
								</div>
								<div id='emerging-companies-filters'>
									<div class="pretty p-default p-round"><input type='radio' name='emergingplayer' value='precovid' id='pcovid' checked />
										<div class="state p-danger"><label for='pcovid'>Old </label> </div>
									</div>
									<div class="pretty p-default p-round"><input type='radio' name='emergingplayer' value='postcovid' id='postcovid' />
										<div class="state p-danger"><label for='postcovid' class=''> New </label></div>
									</div>
									<div class="pretty p-default p-round"> <input type='radio' name='emergingplayer' value='prendpostcovid' id='prendpostcovid' />
										<div class="state p-danger"> <label for='prendpostcovid' class=''> Active </label></div>
									</div>
								</div>
								<div id='leaderboard-checked-text'><input type='checkbox' id='leaderboardtext' /> Show Text</div>
								<div id='insight-result-count-clickedpoint'></div>
							</div>

							<div class='mc-iv-right-top-outer'>
								<div class='mc-iv-right-tester-outer'>
									<div id="tester" style="width:100%;height:100%;"></div>
								</div>
								<div class='chart-comparison-outer'>
								</div>
							</div>
							<div id='insight-navigation'><span id='pre-ins' title='Previous Insight'><img src="images/left-nav.svg" height="15"> Previous</span><span id='nxt-ins' title='Next Insight'>Next <img src="images/right-nav.svg" height="15"></span></div>
						</div>
						<!---->
					</div>
			...
			<div class='bottom'>
				<div class='bottom-inner'>
					...
					<div style='tranlater-div'>
						<a id="google_translate_element"></a>
					</div>
					<div id='pagination'>

						<div><b>Total Records : </b><span id='totalRecord'></span> <select id='perpagerecord' name='perpagerecord'>
								<option>15</option>
								<option>30</option>
								<option>50</option>
							</select>
					...
					<div id='pagination_pubview_outer'>
						...
						<div>
							<ul class='pagination_pubview'>
								<li><a onclick="previous_pub()">
										<image src='images/left-nav.svg' height='15' /> Previous
									</a></li>
								<li><a onclick="next_pub()">Next
										<image src='images/right-nav.svg' height='15' />
									</a></li>
							</ul>
						</div>
					</div>
    ...
    <div class="modal-content">
      ...
      <div class="modal-footer">
		...
		<div class="full-screen hidden-outer flex-container-center">
			<div class='layout-modal'>
				...
				<div>
					...
					<input type='checkbox' name='xaxislabel' id='xaxislabel' /> <label for='xaxislabel'>Show complete X axis tick labels</label>
					<br /><label>X axis tick label angle</label> <select name='xaxislabelangle' id='xaxislabelangle'>
						<option>Auto</option>
						<option>0</option>
						<option>45</option>
						<option>90</option>
					...
					<br /><label>X axis tick font size</label> <select name='xaxislabelfont' id='xaxislabelfont'>
						<option>6</option>
						<option>8</option>
					...
					<br /><input type='checkbox' name='yaxislabel' id='yaxislabel' checked /> <label for='yaxislabel'>Show complete Y axis tick labels</label>
					<br /><label>Y axis tick label angle</label> <select name='yaxislabelangle' id='yaxislabelangle'>
						<option>Auto</option>
						<option>45</option>
						<option>90</option>
						<option>-45</option>
					...
					<br /><label>Y axis tick font size</label> <select name='yaxislabelfont' id='yaxislabelfont'>
						<option>6</option>
						<option>8</option>
						<option selected>10</option>
						<option>12</option>
					...
					<br /><label>Max Bubble size</label><select name='bubbleresizer' id='bubbleresizer'>
						<option>10</option>
						<option>20</option>
						<option>30</option>
						<option>40</option>
		...
		<div class='filter-right-side-view-wrapper-popup' id='filter-right-side-view-wrapper-popup'>
			<div class='filter-right-side-view'>
				...
				<div class='all-filter-options-outer'>
					<form id='filtersform'>
						<div class='filter-label pretty p-default p-round'><input type='checkbox' name='pubdate' value='pubdate' id='flabel-1' />
							<div class='state p-danger'><label for='flabel-1'>Publication Date</label> </div>
						</div>
						<div class='filter-label-opt'>
							<div>
								<select name='pubdate_comparisonoperator' id='pubdate_comparisonoperator'>
									<option>is</option>
									<option>after</option>
						...
						<div class='filter-label pretty p-default p-round'>
							<input type='checkbox' name='pdate' value='pdate' id='flabel-2' />
							<div class='state p-danger'><label for='flabel-2'>Priority Date</label></div>
						</div>
						<div class='filter-label-opt'>
							<div>
								<select name='pdate_comparisonoperator' id='pdate_comparisonoperator'>
									<option>is</option>
									<option>after</option>
									<option>before</option>
									<option>between</option>
						...
						<div class='filter-label pretty p-default p-round'>
							<input type='checkbox' name='appdate' value='appdate' id='flabel-3' />
							<div class='state p-danger'><label for='flabel-3'>Application Date</label> </div>
						</div>
						<div class='filter-label-opt'>
							<div>
								<select name='appdate_comparisonoperator' id='appdate_comparisonoperator'>
									<option>is</option>
									<option>after</option>
									<option>before</option>
									<option>between</option>
						...
						<div class='filter-label pretty p-default p-round'>
							<input type='checkbox' name='updationdate' value='updationdate' id='flabel-4' />
							<div class='state p-danger'><label for='flabel-4'>Archieve Date</label></div>
						</div>
						<div class='filter-label-opt'>
							<div>
								<select name='updation_comparisonoperator' id='updation_comparisonoperator'>
									<option>is</option>
									<option>after</option>
									<option>before</option>
									<option>between</option>
						...
						<div class='filter-label pretty p-default p-round'>
							<input type='checkbox' name='relevancycheck' value='relevancy' id='flabel-5' />
							<div class='state p-danger'><label for='flabel-5'>Relevancy</label> </div>
						</div>
						<div class='filter-label-opt'>
						...
						<div class='filter-label pretty p-default p-round'>
							<input type='checkbox' name='assigneecheck' value='assignee' id='flabel-6' />
							<div class='state p-danger'><label for='flabel-6'>Assignee</label></div>
						</div>
						<div class='filter-label-opt'>
						...
						<div class='filter-label pretty p-default p-round'>
							<input type='checkbox' name='level1check' value='level1' id='flabel-7' />
							<div class='state p-danger'><label for='flabel-7'>Level1</label></div>
						</div>
						<div class='filter-label-opt'>
						...
						<div class='filter-label pretty p-default p-round'>
							<input type='checkbox' name='level2check' value='level2' id='flabel-8' />
							<div class='state p-danger'><label for='flabel-8'>Level2</label></div>
						</div>
						<div class='filter-label-opt'>
						...
						<div class='filter-label pretty p-default p-round'>
							<input type='checkbox' name='level3check' value='level3' id='flabel-9' />
							<div class='state p-danger'><label for='flabel-9'>Level3</label></div>
						</div>
						<div class='filter-label-opt'>
						...
						<div class='filter-label pretty p-default p-round'>
							<input type='checkbox' name='level4check' value='level4' id='flabel-10' />
							<div class='state p-danger'><label for='flabel-10'>Level4</label></div>
						</div>
						<div class='filter-label-opt'>
						...
						<div class='filter-label pretty p-default p-round'>
							<input type='checkbox' name='taggingcheck' value='tagging' id='flabel-11' />
							<div class='state p-danger'><label for='flabel-11'>Tagging</label></div>
						</div>
						<div class='filter-label-opt'>
		...
		<div class='comment-right-side-view-wrapper-popup' id='comment-right-side-view-wrapper-popup'>
			<div class='comment-outer'>
				...
				<div class='comment-post-outer'>
					<div contentEditable="true" placeholder='Add your comment here' required name='comment' id='comment' class='comment_textarea' rows='1'></div>
		...
		<div id='colorboxdiv'>
			...
			<div style="background-color:#F1C40E"></div>
			...
			<div style="background-color:#32D9CC"></div>
			<div style="background-color:#8BC34A"></div>
			<div style="background-color:#FFEB3B"></div>
			<div style="background-color:#da2ad8"></div>
			<div style="background-color:#33495E"></div>
			<div style="background-color:#795548"></div>
			<div style="background-color:#E9503D"></div>
    ...
    height: "2.5em"});*/
				...
				$('#showfilters').on('click', function() {
					...
					if ($(this).hasClass('filter-area-not-expanded')) {
						...
						$('#filter-right-side-view-wrapper-popup').css({
							'top': filtertop,
							'right': 0,
							'bottom': bottomheight
						});
				...
				$('#showcomments').on('click', function() {
					...
					if ($(this).hasClass('comment-area-not-expanded')) {
						...
						var bottomheight = $('.bottom').outerHeight(true);
						$('#comment-right-side-view-wrapper-popup').css({
							'top': cmnttop,
							'right': 0,
							'bottom': bottomheight
						});
				...
				$('body').on('click', '#taggingbtn', function() {
					...
					if ($("#tagging-right-side-view-wrapper-popup").is(":visible")) {
						...
						var bottomheight = $('.bottom').outerHeight(true);
						$('#tagging-right-side-view-wrapper-popup').css({
							'top': cmnttop,
							'left': 5,
							'bottom': bottomheight
						});
				...
				$('body').on('click', '#addcatbtn', function() {
					...
					if ($('#category-right-side-view-wrapper-popup').is(":visible")) {
						...
						var cmnttop = cmntbtn.top + $(this).outerHeight(true) + 5;
						var bottomheight = $('.bottom').outerHeight(true);
						$('#category-right-side-view-wrapper-popup').css({
							'top': cmnttop,
							'right': 0,
							'bottom': bottomheight
						});
				...
				var globalmetadata;
				var comparisonchartx;
				var comparisoncharty;
				var comparisonchartz;
				var comparisonchartclasseshover;
				var comparisonchartmetadata;

				//var charttitle;			
				$('body').on('click', '.getinsight', function() {
					insightclicked = 1;
					exportdata = '';
					exportcomparisonchartdata = '';
					...
					comparisoncharty = [];
					comparisonchartz = [];
					comparisonchartclasseshover = [];
					comparisonchartmetadata = [];
					...
					$('.chart-comparison-outer').html("<div id='processing-div-2'>We are processing your request.... <img src='images/process.gif'/></div>");
					$('.chart-comparison-outer').append("<div id='chart-comparison' style='width:100%;height:100%;'></div>");
					...
					var layout = {
						font: {
							family: 'Telegraf,Bahnschrift, sans-serif'
						},
						colorway: tracecolor,
						title: charttitle,
						paper_bgcolor: '#F6EEE1',
						plot_bgcolor: '#F6EEE1',
						annotations: [{
							xref: 'paper',
							yref: 'paper',
					...
					var config = {
						...
						modeBarButtonsToAdd: [
						                                 ...
						                                 },*/
							{
								...
								icon: {
									'svg': '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24"><path fill="none" d="M0 0h24v24H0z"/><path d="M13 12h3l-4 4-4-4h3V8h2v4zm2-8H5v16h14V8h-4V4zM3 2.992C3 2.444 3.447 2 3.999 2H16l5 5v13.993A1 1 0 0 1 20.007 22H3.993A1 1 0 0 1 3 21.008V2.992z"/></svg>'
								},
								direction: 'up',
								click: function(gd) {
									exportchartdata();
					...
					if (chart == 1) {
						...
						var comparisonreportid = $('#comparison-reportid').val();
						if (comparisonreportid != null && comparisonreportid != '') {
							$.ajax({
								type: "POST",
								url: "retrievetext.php",
								data: fd + "&action=getinsight&reportid=" + comparisonreportid + "&chart=" + chart,
								timeout: 600000,
								dataType: "json",
								success: function(data) {
									$('#processing-div-2').remove();
									if (data != 0) {
										exportcomparisonchartdata = data;
										var x = [];
										var y = [];
										for (i = 0; i < data.length; i++) {
											x.push(data[i].country);
										...
										TESTER = document.getElementById('chart-comparison');
										Plotly.newPlot(
											TESTER,
											[{
												values: y,
										...
										$('#chart-comparison').html("<div>No result set found.</div>");
									}
						...
						$.ajax({
							...
							success: function(data) {
								...
								if (data != 0) {
									...
									if (currentchartid == previouschartid) {
										changecharttype($("#switchchart").val(), charttitle, 'tester', globalx, globaly, globalz, globalmetadata, globalclasseshover);
									} else {
										changecharttype('pie', charttitle, 'tester', globalx, globaly, globalz, globalmetadata, globalclasseshover);
									}
							...
							$.ajax({
								...
								success: function(data) {
									...
									if (data != 0) {
										...
										if (currentchartid == previouschartid) {
											changecharttype($("#switchchart").val(), charttitle, 'chart-comparison', comparisonchartx, comparisoncharty, comparisonchartz, comparisonchartmetadata, comparisonchartclasseshover);
										} else {
											changecharttype('pie', charttitle, 'chart-comparison', comparisonchartx, comparisoncharty, comparisonchartz, comparisonchartmetadata, comparisonchartclasseshover);
										}
							...
							success: function(data) {
								...
								if (data != 0) {
									...
									globalmetadata = {
										...
										'x': 'pdate',
										'y': 'count',
										'xcol': 'Priority/Filing Year',
										'ycol': 'Count'
									};
									if (currentchartid == previouschartid) {
										changecharttype($("#switchchart").val(), charttitle, 'tester', globalx, globaly, globalz, globalmetadata, globalclasseshover);
									} else {
										changecharttype('bar', charttitle, 'tester', globalx, globaly, globalz, globalmetadata, globalclasseshover);
									}
						...
						if (comparisonreportid != null && comparisonreportid != '') {
							$.ajax({
								type: "POST",
								url: "retrievetext.php",
								data: fd + "&action=getinsight&reportid=" + comparisonreportid + "&chart=" + chart + "&pdateopt=" + $('#priority-year-opt').val(),
								timeout: 600000,
								dataType: "json",
								success: function(data) {
									...
									if (data != 0) {
										...
										if (currentchartid == previouschartid) {
											...
											changecharttype('bar', charttitle, 'chart-comparison', comparisonchartx, comparisoncharty, comparisonchartz, comparisonchartmetadata, comparisonchartclasseshover);
										}
						...
						if (comparisonreportid != null && comparisonreportid != '') {
							$.ajax({
								type: "POST",
								url: "retrievetext.php",
								data: fd + "&action=getinsight&reportid=" + comparisonreportid + "&chart=" + chart + "&topvalue=" + $("#top-comp-opt").val(),
								timeout: 600000,
								dataType: "json",
								success: function(data) {
									$('#processing-div-2').remove();
						...
						if (comparisonreportid != null && comparisonreportid != '') {
							$.ajax({
								type: "POST",
								url: "retrievetext.php",
								data: "action=getinsight&reportid=" + comparisonreportid + "&chart=" + chart,
								timeout: 600000,
								dataType: "json",
								success: function(result) {
									$('#processing-div-2').remove();
									if (result != 0) {
									 ...
									 config);*/
										...
										var margin = {
											...
											width = $('#chart-comparison').width() - margin.right - margin.left,
											height = $('#chart-comparison').height() * 2 - margin.top - margin.bottom;
										...
										var svg = d3.select("#chart-comparison").append("svg")
											//.attr("width", width + margin.right + margin.left)
											//.attr("height", height + margin.top + margin.bottom)
											.attr("width", "100%")
											.attr("height", "200%")
							...
							success: function(data) {
								...
								if (data != 0) {
									...
									if (currentchartid == previouschartid) {
										...
										changecharttype('bubble', charttitle, 'tester', globalx, globaly, globalz, globalmetadata, globalclasseshover);
									}
								...
								success: function(data) {
									...
									if (data != 0) {
										...
										if (currentchartid == previouschartid) {
											...
											changecharttype('bubble', charttitle, 'chart-comparison', comparisonchartx, comparisoncharty, comparisonchartz, comparisonchartmetadata, comparisonchartclasseshover);
										}
						...
						if (comparisonreportid != null && comparisonreportid != '') {
							$.ajax({
								type: "POST",
								url: "retrievetext.php",
								data: fd + "&action=getinsight&reportid=" + comparisonreportid + "&chart=" + chart + "&level=" + encodeURIComponent($(this).data('toplevel')) + "&hlevel=" + hlevel + "&pdateopt=" + $('#priority-year-opt').val(),
								timeout: 600000,
								dataType: "json",
								success: function(data) {
									$('#processing-div-2').remove();
						...
						if (comparisonreportid != null && comparisonreportid != '') {
							$.ajax({
								type: "POST",
								url: "retrievetext.php",
								data: fd + "&action=getinsight&reportid=" + comparisonreportid + "&chart=" + chart + "&level=" + encodeURIComponent($(this).data('toplevel')) + "&hlevel=" + hlevel,
								timeout: 600000,
								dataType: "json",
								success: function(data) {
						...
						if (comparisonreportid != null && comparisonreportid != '') {
							$.ajax({
								type: "POST",
								url: "retrievetext.php",
								data: fd + "&action=getinsight&reportid=" + comparisonreportid + "&chart=" + chart + "&typeofassignee=" + encodeURIComponent($(this).data('typeofassignee')) + "&topvalue=" + $("#top-comp-opt").val(),
								timeout: 600000,
								dataType: "json",
								success: function(data) {
									$('#processing-div-2').remove();
						...
						if (comparisonreportid != null && comparisonreportid != '') {
							$.ajax({
								type: "POST",
								url: "retrievetext.php",
								data: fd + "&action=getinsight&reportid=" + comparisonreportid + "&chart=" + chart + "&level=" + encodeURIComponent($(this).data('toplevel')) + "&topvalue=" + $("#top-comp-opt").val() + "&hlevel=" + hlevel,
								timeout: 600000,
								dataType: "json",
								success: function(data) {
						...
						if (comparisonreportid != null && comparisonreportid != '') {
							$.ajax({
								type: "POST",
								url: "retrievetext.php",
								data: fd + "&action=getinsight&reportid=" + comparisonreportid + "&chart=" + chart + "&topvalue=" + $("#top-comp-opt").val() + "&pdateopt=" + $('#priority-year-opt').val(),
								timeout: 600000,
								dataType: "json",
								success: function(data) {
									$('#processing-div-2').remove();
						...
						if (comparisonreportid != null && comparisonreportid != '') {
							$.ajax({
								type: "POST",
								url: "retrievetext.php",
								data: fd + "&action=getinsight&reportid=" + comparisonreportid + "&chart=" + chart + "&level=" + encodeURIComponent($(this).data('toplevel')) + "&scoringopt=" + $('#scoring-opt').val() + "&activeauthopt=" + $('#active-auth-opt').val() + "&hlevel=" + hlevel + "&pdateopt=" + $('#priority-year-opt').val(),
								timeout: 600000,
								dataType: "json",
								success: function(data) {
									$('#processing-div-2').remove();
						...
						if (comparisonreportid != null && comparisonreportid != '') {
							$.ajax({
								type: "POST",
								url: "retrievetext.php",
								data: fd + "&action=getinsight&reportid=" + comparisonreportid + "&chart=" + chart + "&level=" + encodeURIComponent($(this).data('toplevel')) + "&scoringopt=" + $('#scoring-opt').val() + "&activeauthopt=" + $('#active-auth-opt').val() + "&topvalue=" + $("#top-comp-opt").val() + "&hlevel=" + hlevel,
								timeout: 600000,
								dataType: "json",
								success: function(data) {
									$('#processing-div-2').remove();
							...
							success: function(data) {
								...
								if (data != 0) {
									...
									globalmetadata = {
										...
										'x': 'assignee',
										'y': 'typeofassignee',
										'typeofassignee': typeofassignee,
										'xcol': 'Normalized Portfolio size(Count)',
										'ycol': 'Normalized Portfolio strength(Score)',
										'zcol': 'Assignee Name'
									};
									if (currentchartid == previouschartid) {
										changecharttype($("#switchchart").val(), charttitle, 'tester', globalx, globaly, globalz, globalmetadata, globalclasseshover);
									} else {
										changecharttype('leaderboard', charttitle, 'tester', globalx, globaly, globalz, globalmetadata, globalclasseshover);
									}
								} else {
									$('#tester').html("<div>No result set found.</div>");
								}
						...
						if (comparisonreportid != null && comparisonreportid != '') {
							$.ajax({
								type: "POST",
								url: "retrievetext.php",
								data: fd + "&action=getinsight&reportid=" + comparisonreportid + "&chart=" + chart + "&typeofassignee=" + encodeURIComponent($(this).data('typeofassignee')) + "&scoringopt=" + $('#scoring-opt').val() + "&activeauthopt=" + $('#active-auth-opt').val() + "&topvalue=" + $("#top-comp-opt").val(),
								timeout: 600000,
								dataType: "json",
								success: function(data) {
									$('#processing-div-2').remove();
									if (data != 0) {
										...
										if (currentchartid == previouschartid) {
											...
											changecharttype('leaderboard', charttitle, 'chart-comparison', comparisonchartx, comparisoncharty, comparisonchartz, comparisonchartmetadata, comparisonchartclasseshover);
										}
									} else {
										$('#chart-comparison').html("<div>No result set found.</div>");
									}
							...
							success: function(data) {
								...
								if (data != 0) {
									...
									if (currentchartid == previouschartid) {
										...
										changecharttype('3Dbar', charttitle, 'tester', globalx, globaly, globalz, globalmetadata, globalclasseshover);
									}
									...
									if (data != 0) {
										...
										/*$.each(data, function(key, value)
									  {
										...
										if (currentchartid == previouschartid) {
											...
											changecharttype('3Dbar', charttitle, 'chart-comparison', comparisonchartx, comparisoncharty, comparisonchartz, comparisonchartmetadata, comparisonchartclasseshover);
										}
						...
						$.ajax({
							...
							success: function(data) {
								...
								if (data != 0) {
									...
									globalmetadata = {
										'drilldown': 'enable',
										'x': 'overalllegalstatus',
										'y': 'count',
										'xcol': 'Overall Family Leagal Status',
										'ycol': 'Count'
									};
									if (currentchartid == previouschartid) {
										changecharttype($("#switchchart").val(), charttitle, 'tester', globalx, globaly, globalz, globalmetadata, globalclasseshover);
							...
							success: function(data) {
								...
								if (data != 0) {
									...
									globalmetadata = {
										'drilldown': 'enable',
										'x': 'typeofassignee',
										'y': 'overalllegalstatus',
										'xcol': 'Type of assignee',
										'ycol': 'Overall Family Legal Status',
										'zcol': 'Count'
									};
						...
						$.ajax({
							...
							success: function(data) {
								...
								if (data != 0) {
									...
									globalmetadata = {
										'drilldown': 'enable',
										'x': 'familymemberslegalstatus',
										'y': 'count',
										'xcol': 'Family members legal status',
										'ycol': 'Count'
									};
							...
							success: function(data) {
								...
								if (data != 0) {
									...
									globalmetadata = {
										'drilldown': 'enable',
										'x': 'assignee',
										'y': 'familymemberslegalstatus',
										'xcol': 'Assignee Name',
										'ycol': 'Family members legal status',
										'zcol': 'Count'
									};
									if (currentchartid == previouschartid) {
										changecharttype($("#switchchart").val(), charttitle, 'tester', globalx, globaly, globalz, globalmetadata, globalclasseshover);
						...
						if (comparisonreportid != null && comparisonreportid != '') {
							$.ajax({
								type: "POST",
								url: "retrievetext.php",
								data: fd + "&action=getinsight&reportid=" + comparisonreportid + "&chart=" + chart + "&level=" + encodeURIComponent($(this).data('toplevel')),
								timeout: 600000,
								dataType: "json",
								success: function(data) {
									$('#processing-div-2').remove();
							...
							$.ajax({
								...
								success: function(data) {
									...
									if (data != 0) {
										...
										comparisoncharty = y;
										comparisonchartz = z;
										comparisonchartclasseshover = classdef;
										comparisonchartmetadata = {
											'drilldown': 'enable',
											'x': 'ipc',
											'y': 'assignee'
								...
								success: function(data) {
									...
									if (data != 0) {
										...
										if (currentchartid == previouschartid) {
											...
											changecharttype('3Dbar', charttitle, 'comparisonreportid', comparisonchartx, comparisoncharty, comparisonchartz, comparisonchartmetadata, comparisonchartclasseshover);
										}
									} else {
										$('#comparisonreportid').html("<div>No result set found.</div>");
									}
							...
							success: function(data) {
								...
								if (data != 0) {
									...
									if (currentchartid == previouschartid) {
										...
										changecharttype('3Dline', charttitle, 'tester', globalx, globaly, globalz, globalmetadata, globalclasseshover);
									}
								...
								success: function(data) {
									...
									if (data != 0) {
										...
										if (currentchartid == previouschartid) {
											...
											changecharttype('3Dline', charttitle, 'chart-comparison', comparisonchartx, comparisoncharty, comparisonchartz, comparisonchartmetadata, comparisonchartclasseshover);
										}
							...
							success: function(data) {
								...
								if (data != 0) {
									...
									globalmetadata = {
										...
										'y': 'headquarter',
										'z': 'typeofassignee',
										'zvalue': typeofassignee,
										'xcol': 'Priority/Filing Year',
										'ycol': 'Companies Region/headquarter',
										'zcol': 'Count'
									};
						...
						if (comparisonreportid != null && comparisonreportid != '') {
							$.ajax({
								type: "POST",
								url: "retrievetext.php",
								data: fd + "&action=getinsight&reportid=" + comparisonreportid + "&chart=" + chart + "&typeofassignee=" + encodeURIComponent($(this).data('typeofassignee')) + "&pdateopt=" + $('#priority-year-opt').val(),
								timeout: 600000,
								dataType: "json",
								success: function(data) {
									$('#processing-div-2').remove();
							...
							success: function(data) {
								...
								if (data != 0) {
									...
									globalmetadata = {
										...
										'z': 'typeofassignee',
										'zvalue': typeofassignee,
										'xcol': 'Companies region/headquarter',
										'ycol': 'Count'
									};
						...
						if (comparisonreportid != null && comparisonreportid != '') {
							$.ajax({
								type: "POST",
								url: "retrievetext.php",
								data: fd + "&action=getinsight&reportid=" + comparisonreportid + "&chart=" + chart + "&typeofassignee=" + encodeURIComponent($(this).data('typeofassignee')),
								timeout: 600000,
								dataType: "json",
								success: function(data) {
									$('#processing-div-2').remove();
						...
						if (comparisonreportid != null && comparisonreportid != '') {
							$.ajax({
								type: "POST",
								url: "retrievetext.php",
								data: fd + "&action=getinsight&reportid=" + comparisonreportid + "&chart=" + chart + "&typeofassignee=" + encodeURIComponent($(this).data('typeofassignee')) + "&topvalue=" + $("#top-comp-opt").val() + "&pdateopt=" + $('#priority-year-opt').val(),
								timeout: 600000,
								dataType: "json",
								success: function(data) {
									$('#processing-div-2').remove();
				...
				$("#updatecmtbtn").click(function(e) {
					...
					$('#comment .c-mention').each(function() {
						var Email = $(this).data('email');
						$array = "{" + Email + ":'0'}";
						// $temparray =  json_decode($array);
						mailWithStatus.push($array);
					});
					...
					$.ajax({
						type: "POST",
						url: "getcomments.php",
						data: "comment=" + comment + "&action=addcomment&reportid=" + reportid + "&chartid=" + chartid + "&mailWithStatus=" + mailWithStatus + "&kindofcomment=" + kindofcomment,
						dataType: "json",
						success: function(data) {
						...
						success: function(data) {
							...
							if (data == 0) {
								...
								for (i = 0; i < data.length; i++) {
									...
									content = content + "<div><div style='text-align:right;'> " + data[i].creationdate + "</div><div class='" + data[i].flag + "'><div class='msg-outer'><div class='msg-createdby'>" + data[i].commentby + "</div>" + data[i].comment.replace(/\n/g, "<br />") + "</div></div></div>";
									// console.log(data[data.length - 1].useremails);
									


									if (data[i].current && data[i].useremails) {

										// data[i].value2.forEach(function(item){
										// 	 console.log(item.includes(data[i].clientuser));
										// 	if(item.includes(data[i].clientuser)){
								...
								$('#commentbody').append(content);
									...
									var notification = "<span class='counter'>" + notify + "</span></label>";
									$("#showcomments").append(notification);
				...
				function exportchartdata(chartid) {
					...
					if (chartid == 'tester') {
						...
						return false;
					} else if (chartid == 'chart-comparison') {
						var x = new CSVExport(exportcomparisonchartdata);
						return false;
					}
				}

				function changecharttype(charttype, charttitle, chartposid, globalx, globaly, globalz, globalmetadata, globalclasseshover) {
					...
					var layout = {
						...
						font: {
							family: 'Bauziet, Telegraf, Bahnschrift, sans-serif'
						},
						colorway: tracecolor,
						title: {
					...
					var config = {
						...
						modeBarButtonsToAdd: [
						                                 ...
						                                 },*/
							{
								...
								direction: 'up',
								click: function(chartposid) {
									//alert(chartposid.id);
									exportchartdata(chartposid.id);
								}
							}
						]
					};

					var searchreportid = reportid;
					$('#' + chartposid).remove();
					if (chartposid == 'tester') {
						$('.mc-iv-right-tester-outer').append("<div id='tester' style='width:100%;height:100%;'></div>");
					} else if (chartposid == 'chart-comparison') {
						searchreportid = $('#comparison-reportid').val();
						$('.chart-comparison-outer').append("<div id='chart-comparison' style='width:100%;height:100%;'></div>");
					}
					//
					//
					...
					if (charttype == 'line') {
						...
						if (globalmetadata.drilldown == 'enable') {
							TESTER.on('plotly_click', function(data) {
								...
								setTimeout(function() {
									...
									var redirecturl = 'showdata_trial.php?&' + pts.join('&') + "&" + ptsval.join('&') + "&action=showdata&reportid=" + searchreportid + "&" + fd;
									if (typeof globalmetadata.x !== "undefined") {
						...
						var data = [{
							...
							marker: {
								size: globalz,
								sizeref: 2 * maxbubblesize / (bubblesizeset ** 2),
								sizemode: 'area',
								sizemin: 10
							},
							transforms: [{
						...
						$('#' + chartposid).append('<div class="ins-datatbl-outer"><table class="table ins-datatbl"><thead>' + tableheader[0] + '</thead><tbody>' + tablebody + '</tbody></table></div>')
				...
				$('#switchchart').on('change', function() {
					...
					//var globalx = gd.data[0].x;
					//var globaly = gd.data[0].y;
					//var globalz = gd.data[0].z;
					//var globalmetadata =[];
					//var globalclasseshover =[];
					changecharttype($(this).val(), charttitle, 'tester', globalx, globaly, globalz, globalmetadata, globalclasseshover);
					//alert(comparisonchartx);
					changecharttype($(this).val(), charttitle, 'chart-comparison', comparisonchartx, comparisoncharty, comparisonchartz, comparisonchartmetadata, comparisonchartclasseshover);
				});

				$('#comparison-reportid').on('change', function() {
					if ($(this).val() != '') {
						$(".ins-selected").trigger('click');
						$('.chart-comparison-outer').show();
						window.dispatchEvent(new Event('resize'));
						//changecharttype($(this).val(),charttitle,'chart-comparison',globalx,globaly,globalz,globalmetadata,globalclasseshover);
					} else {
						$('.chart-comparison-outer').hide();
						window.dispatchEvent(new Event('resize'));
				...
				for (var i = 0; i < palette1.length; i++) {
					$('#p1').append("<div class='color-picker' data-colnum='" + i + "' style='display:inline-block;width:10px;height:10px;background-color:" + palette1[i] + "'></div>");
				}
				...
				for (var i = 0; i < palette2.length; i++) {
					$('#p2').append("<div class='color-picker' data-colnum='" + i + "' style='display:inline-block;width:10px;height:10px;background-color:" + palette2[i] + "'></div>");
				}
				var palette3 = ['#228b22', '#00008b', '#b03060', '#ff4500', '#ffff00', '#deb887', '#00ff00', '#00ffff', '#ff00ff', '#6495ed'];
				for (var i = 0; i < palette3.length; i++) {
					$('#p3').append("<div class='color-picker' data-colnum='" + i + "' style='display:inline-block;width:10px;height:10px;background-color:" + palette3[i] + "'></div>");
				}
				var palette4 = ['#003f5c', '#2f4b7c', '#665191', '#a05195', '#d45087', '#f95d6a', '#ff7c43', '#ffa600'];
				for (var i = 0; i < palette4.length; i++) {
					$('#p4').append("<div class='color-picker' data-colnum='" + i + "' style='display:inline-block;width:10px;height:10px;background-color:" + palette4[i] + "'></div>");
				}
				var palette5 = ['#004c6d', '#25617e', '#3f768f', '#588ca0', '#72a3b2', '#8cb9c4', '#a8d0d7', '#c4e7ea', '#e1ffff'];
				for (var i = 0; i < palette5.length; i++) {
					$('#p5').append("<div class='color-picker' data-colnum='" + i + "' style='display:inline-block;width:10px;height:10px;background-color:" + palette5[i] + "'></div>");
				}
				...
				for (var i = 0; i < palette6.length; i++) {
					$('#p6').append("<div class='color-picker' data-colnum='" + i + "' style='display:inline-block;width:10px;height:10px;background-color:" + palette6[i] + "'></div>");
				}
				...
				for (var i = 0; i < palette7.length; i++) {
					$('#p7').append("<div class='color-picker' data-colnum='" + i + "' style='display:inline-block;width:10px;height:10px;background-color:" + palette7[i] + "'></div>");
				}
				...
				let sources = document.querySelectorAll('.color-picker');
				let init = true;
				...
				for (let i = 0, j = sources.length; i < j; ++i) {
					(new CP(sources[i])).on('change', function(r, g, b, a) {
						...
						this.source.style.background = this.color(r, g, b, a);
						colornum = this.source.getAttribute('data-colnum');
						var parentspan = this.source.parentNode.id;
						if (parentspan == 'p1') {
							palette1.splice(colornum, 1, this.color(r, g, b, a));
						} else if (parentspan == 'p2') {
							palette2.splice(colornum, 1, this.color(r, g, b, a));
						} else if (parentspan == 'p3') {
							palette3.splice(colornum, 1, this.color(r, g, b, a));
						} else if (parentspan == 'p4') {
							palette4.splice(colornum, 1, this.color(r, g, b, a));
						} else if (parentspan == 'p5') {
							palette5.splice(colornum, 1, this.color(r, g, b, a));
						} else if (parentspan == 'p6') {
							palette6.splice(colornum, 1, this.color(r, g, b, a));
						} else if (parentspan == 'p7') {
							palette7.splice(colornum, 1, this.color(r, g, b, a));
						}

						changecharttype($("#switchchart").val(), charttitle, 'tester', globalx, globaly, globalz, globalmetadata, globalclasseshover);
						changecharttype($("#switchchart").val(), charttitle, 'chart-comparison', comparisonchartx, comparisoncharty, comparisonchartz, comparisonchartmetadata, comparisonchartclasseshover);
						//$('.clicked-color').css({'background':this.color(r, g, b, a)}).addClass('clicked-color');
					});
				}

				$('.color-picker').on('click', function() {
					$('.color-picker').removeClass('clicked-color');
					$(this).addClass('clicked-color');
					init = false;

				});

				var tribute = new Tribute({
					values: <?php echo json_encode($mentiondata); ?>,
					selectTemplate: function(item) {
						return (
							'<span class="c-mention" contenteditable="false" data-email="' + item.original.email + '">@' +
							item.original.value +
							'</span>'
						);
					}
				...
				<?php
				if (isset($_GET['chart-id']) && trim($_GET['chart-id']) != '') {
				?>

					//sectiontoshow_after_applyingfilter = 2;
					$('#showinsights').trigger('click');

					var charttoopen = '<?php echo $_GET['chart-id']; ?>';
					$('#' + charttoopen).trigger('click');
